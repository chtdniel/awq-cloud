...
try {
  ...
  await page.locator('#ll-save-btn').click();
  await expect(page.locator('#ll-kpi-total')).toHaveText('5');
  ...
  console.log('Browser QA passed: real login, preview, edited paste, replace, append, merge, refresh/read-back, row delete, validation and clear; one write per click; no page errors.');
} finally {
  await browser.close();
